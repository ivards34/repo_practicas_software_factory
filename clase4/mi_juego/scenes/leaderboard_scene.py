...
def __init__(self, manager, after_game: bool, last_score: int
| None):
...
def on_enter(self):n self.font_title =
...
bw, bh = 260, 50
cx = w // 2; y0 = 320; sep = 60
self.btn_play_again = Button(pygame.Rect(cx - bw//2, y0,

bw, bh), "Volver a jugar", self.font_row)

self.btn_menu = Button(pygame.Rect(cx - bw//2, y0 + sep,

bw, bh), "Volver al menú", self.font_row)

self.btn_quit = Button(pygame.Rect(cx - bw//2, y0 + 2*sep,

bw, bh), "Salir", self.font_row)
...
w, _ = screen.get_size()
title = self.font_title.render("Top 10 Puntajes", True,
...
line = f"{idx:>2}. {r['nombre']:<12}

{r['puntaje']:>5} (Nivel {r['nivel']})"
...
info = self.font_info.render(f"Tu último puntaje:

{self.last_score}", True, (180, 220, 255))

screen.blit(info, info.get_rect(center=(w//2, 285)))

for b in (self.btn_play_again, self.btn_menu,
